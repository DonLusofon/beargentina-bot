...
async def _process_delete_input(user_id: int, text: str, message: Message) -> None:
...
async def notify_seller(bot: Bot, item: CatalogItem, user: Message) -> bool:
...
async def notify_admins(bot: Bot, item: CatalogItem, user: Message) -> None:
...
async def start_new_command(message: Message) -> None:
...
async def handle_new_flow(message: Message) -> None:
...
async def start_delete_command(message: Message) -> None:
    if not message.from_user or message.from_user.id not in config.ADMIN_CHAT_IDS:
        await message.answer("Команда доступна только главным администраторам.")
        return
    if not catalog:
...
async def handle_delete_flow(message: Message) -> None:
    user = message.from_user
...
async def handle_start(message: Message, bot: Bot, command: Optional[CommandObject] = None) -> None:
    ...
    notified = await notify_seller(bot, item, message)
    await notify_admins(bot, item, message)
    await message.answer(build_customer_reply(item))


async def main(print_links: bool = False) -> None:
    global catalog
    ...
    dp.message.register(start_new_command, Command("new"))
    dp.message.register(start_delete_command, Command("delete"))
    dp.message.register(handle_new_flow)
    dp.message.register(handle_delete_flow)
...
if __name__ == "__main__":
    ...
    asyncio.run(main(print_links=args.print_links))
